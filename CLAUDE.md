...
**Co-management module (FAO DACMS).** `/groups` (list) + `/groups/[id]` (detail, `src/views/GroupDetail.jsx` + `src/components/groups/*`) implement FAO's aquaculture co-management framework: groups (committees/cooperatives/associations) → linked operator members (women/youth representation), co-management plan + SMART indicators (baseline/target/current), meetings with minutes, conflict & compliance register, DACMS Annex-1 self-assessment (criteria in `src/data/dacms.js`, sourced from FAO cd0722en — treat like other verified data, don't invent criteria), and GeoJSON zone boundaries (drawn in `ZoneMapEditor`, rendered as a polygon layer on `/map`). Schema section 10 in `supabase/schema.sql`; `community_overview()` is a security-definer RPC returning anonymized cross-agent aggregates (transparency) — it must never expose row-level data or PII.
